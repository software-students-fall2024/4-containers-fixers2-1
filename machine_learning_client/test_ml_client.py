...
def mock_db():
    ...
    """
    with patch(
        "machine_learning_client.ml_client.emotion_data_collection", autospec=True
    ) as mock_collection:
...
def test_model_not_loaded(client, mock_db):
    ...
    with patch(
        ...
        assert "error" in response_data
        assert (
            "Emotion detection model not loaded" in response_data["error"]
...
def test_detect_emotion_invalid_image(client):
    ...
    file_storage = FileStorage(
        stream=BytesIO(invalid_data), filename="invalid.jpg", content_type="image/jpeg"
    )
